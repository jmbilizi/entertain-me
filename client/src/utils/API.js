...
export default {
  ...
  // API call for movie rating -- need to credit    COMBINED
  ...
  relatedMoviesSearch: function (id) { 
    ...
    ratingSearch: function (id, mediaType) {
      return axios.get(
        `https://api.themoviedb.org/3/${mediaType}/${id}/${mediaType === 'tv' ? 'content_ratings' : 'release_dates' }?api_key=${apiKey}&language=en-US`
      );
    },
...
celebSearch: function (userInput) {
  return axios.get(
    `https://api.themoviedb.org/3/person/${userInput}?api_key=${apiKey}&language=en-US`
  );
},
